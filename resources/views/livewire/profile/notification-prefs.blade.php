    ...
    <section class="space-y-4 rounded-[8px] bg-white p-5" style="border: 1px solid var(--border);">
        @foreach ([
            ...
            <label class="flex items-center justify-between border-b pb-4 last:border-b-0 last:pb-0" style="border-color: var(--border);">
                ...
                <button type="button" wire:click="$toggle('{{ $key }}')" class="relative h-7 w-12 transition {{ $$key ? 'bg-teal' : 'bg-slate-300' }}" style="border-radius: 999px;">
                    <span class="absolute top-1 h-5 w-5 rounded-full bg-white transition {{ $$key ? 'left-6' : 'left-1' }}"></span>
                </button>
            </label>
        @endforeach

        <button type="button" wire:click="save" class="tmc-button-gold w-full">Save Preferences</button>
    </section>
</div>
