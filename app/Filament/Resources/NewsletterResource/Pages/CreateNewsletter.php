<?php

namespace App\Filament\Resources\NewsletterResource\Pages;

use App\Filament\Resources\NewsletterResource;
use App\Jobs\SendNewsletterEmailJob;
use App\Services\AuditLogService;
use Filament\Actions;
use Filament\Notifications\Notification;
use Filament\Resources\Pages\CreateRecord;

class CreateNewsletter extends CreateRecord
{
    protected static string $resource = NewsletterResource::class;

    protected function getHeaderActions(): array
    {
        return [
            Actions\Action::make('saveDraft')
                ->label('Save Draft')
                ->icon('heroicon-o-document')
                ->color('gray')
                ->requiresConfirmation()
                ->modalHeading('Save as Draft')
                ->modalDescription('This newsletter will be saved as a draft and will not be sent or scheduled.')
                ->action(function () {
                    $data = $this->form->getState();
                    $data['created_by'] = auth()->id();
                    $data['status'] = 'draft';
                    $record = $this->getModel()::create($data);

                    AuditLogService::log('newsletter_created', $record, [], $record->only([
                        'subject', 'target_audience', 'status',
                    ]));

                    Notification::make()
                        ->title('Draft saved')
                        ->success()
                        ->send();

                    $this->redirect($this->getResource()::getUrl('edit', ['record' => $record]));
                }),
        ];
    }

    protected function mutateFormDataBeforeCreate(array $data): array
    {
        $data['created_by'] = auth()->id();

        if (! empty($data['schedule_at'])) {
            $data['status'] = 'scheduled';
        } else {
            $data['status'] = 'sending';
        }

        return $data;
    }

    protected function afterCreate(): void
    {
        AuditLogService::log('newsletter_created', $this->record, [], $this->record->only([
            'subject', 'target_audience', 'status',
        ]));

        if ($this->record->status === 'sending') {
            SendNewsletterEmailJob::dispatch($this->record);
        }
    }
}
